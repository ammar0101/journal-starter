...
async def update_entry(
    ...
    tests/test_api.py.
    """
    # Convert the validated model back into a dictionary for the service
...
async def delete_entry(entry_id: str, entry_service: EntryService = Depends(get_entry_service)):
    ...
    """
    # 1. Await the service to fetch the entry
    entry = await entry_service.get_entry(entry_id)

    # 2. If the entry is not found, raise a 404 error
    if entry is None:
        raise HTTPException(status_code=404, detail="Entry not found")

    # 3. Await the service to actually delete the entry
    await entry_service.delete_entry(entry_id)

    # 4. Return the exact success dictionary required
